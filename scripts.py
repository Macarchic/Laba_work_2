...
class Vector:
    def __init__(self, array_atom, new_names, lines):
        self.array_atom = array_atom
        self.result = []
        indexes = []
        indexes.append(new_names.index(self.array_atom[0]))
        indexes.append(new_names.index(self.array_atom[1]))
        indexes.append(new_names.index(self.array_atom[2]))
        indexes.append(new_names.index(self.array_atom[3]))

        for check in range(0, 3):
            parts1 = lines[indexes[check]]
            parts2 = lines[indexes[check+1]]
            v = [float(parts2[1]) - float(parts1[1]), float(parts2[2]) - float(parts1[2]), float(parts2[3]) - float(parts1[3])]
